...
checksum: build
	${CHECKSUM} ${BINARY} > ${BINARY}.${CHECKSUM_EXT}
...
tar: checksum
	...
	tar cvzf ${ARTIFACTS_DIR}/${BINARY}-${VERSION}.tar.gz ${BINARY} ${BINARY}.${CHECKSUM_EXT}
	rm -rf ${BINARY} ${BINARY}.${CHECKSUM_EXT}
#
# clear binaries generated by install or build targets
clean:
	if [ -f ${BINARY} ] ; then rm -f ${BINARY} ; rm -f $$GOPATH/bin/${BINARY} ; fi
	if [ -f ${BINARY}.${CHECKSUM_EXT} ] ; then rm -f ${BINARY}.${CHECKSUM_EXT} ; fi
