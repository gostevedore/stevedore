...
func (credential *Credential) IsValid() (bool, error) {
	...
	if credential == nil {
		return false, errors.New(errContext, "Invalid credential. Credential is nil")
	}
	...
	// invalid credentials
	if credential.Username != "" && credential.Password == "" {
		return false, errors.New(errContext, "Invalid credential. Missing password")
	}

	if credential.AWSAccessKeyID != "" && credential.AWSSecretAccessKey == "" {
		return false, errors.New(errContext, "Invalid credential. Missing AWS secret access key")
	}

	if credential.Password != "" && credential.Username == "" {
		return false, errors.New(errContext, "Invalid credential. Missing username")
	}

	if credential.AWSSecretAccessKey != "" && credential.AWSAccessKeyID == "" {
		return false, errors.New(errContext, "Invalid credential. Missing AWS access key")
	}

	return false, errors.New(errContext, "Invalid credential. Unknown reason")
}
